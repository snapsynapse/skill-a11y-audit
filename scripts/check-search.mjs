...
// previous_version: 3
// change_summary: Adds offline required-file and exact-route JSON-LD validation.
...
function dateModified(html, rel, pathname, sitemapLastmod) {
    ...
    if (!dates.length) report('defect', `${rel}: TechArticle JSON-LD is missing dateModified`);
    else if (!dates.includes(sitemapLastmod)) report('defect', `${rel}: TechArticle dateModified ${dates.join(', ')} does not equal sitemap lastmod ${sitemapLastmod}`);
}
...
for (const entry of entries) {
    ...
    dateModified(html, rel, url.pathname, entry.lastmod);
}
