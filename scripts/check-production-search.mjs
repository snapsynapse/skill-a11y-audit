...
// previous_version: 3
// change_summary: Adds exact-route JSON-LD support and v4 configuration compatibility.
...
function hasHosted404Marker(body) {
    return (config.hosted404Patterns || []).some(pattern => body.toLowerCase().includes(String(pattern).toLowerCase()));
}
...
if (production) {
    for (const pathname of config.expectedNotFound || []) {
        ...
        if (!type.toLowerCase().includes('text/html')) report('defect', `${target}: expected HTML 404 content-type, received ${type || '(missing)'}`);
        if ((config.hosted404Patterns || []).length && !hasHosted404Marker(body)) report('defect', `${target}: hosted 404 body did not match an expected marker`);
    }
}
...
function structuredDateModified(value, dates = []) {
    if (Array.isArray(value)) {
        for (const item of value) structuredDateModified(item, dates);
    } else if (value && typeof value === 'object') {
        const types = Array.isArray(value['@type']) ? value['@type'] : [value['@type']];
        if (types.includes('TechArticle') && typeof value.dateModified === 'string') dates.push(value.dateModified.slice(0, 10));
        for (const child of Object.values(value)) structuredDateModified(child, dates);
    }
    return dates;
}

function dateModified(html, url, sitemapLastmod) {
    const pathname = new URL(url).pathname;
    if (!(config.dateModifiedRequiredPaths || []).includes(pathname)) return;
    ...
    for (const block of html.matchAll(/<script\b[^>]*type=["']application\/ld\+json["'][^>]*>([\s\S]*?)<\/script>/gi)) {
        try { structuredDateModified(JSON.parse(block[1]), dates); } catch { /* reported by jsonLd */ }
    }
    if (!dates.length) report('defect', `${url}: TechArticle JSON-LD is missing dateModified`);
    else if (!dates.includes(sitemapLastmod)) report('defect', `${url}: TechArticle dateModified ${dates.join(', ')} does not equal sitemap lastmod ${sitemapLastmod}`);
}
...
async function checkPage(canonical) {
    ...
    jsonLd(html, target);
    dateModified(html, target, sitemapLastmods.get(canonical));
}
